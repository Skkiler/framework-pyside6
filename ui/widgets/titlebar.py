...
class IconWidget(QWidget):
    def __init__(self, icon: Optional[Union[str, QPixmap, QIcon]] = None,
        ...
        # tamanho lógico fixo do “slot” do ícone
    ...
    def _normalize(self, icon: Optional[Union[str, QPixmap, QIcon]]) -> Optional[QIcon]:
        if not icon:
            # fallback: usa o windowIcon, prática comum em frameless
            win = self.window()
    ...
    def paintEvent(self, e):
        ...
        # canvas físico (lógico×DPR)
        phys_w, phys_h = int(self._icon_size * dpr), int(self._icon_size * dpr)
        pm = self._icon.pixmap(phys_w, phys_h)  # QIcon escolhe o melhor frame
        if pm.isNull():
            return
        try:
        ...
        # alvo lógico (centralizado verticalmente)
...
class TitleBar(QWidget):
    ...
    def __init__(self, title: str = "", parent: QWidget | None = None,
        ...
        # ---- Ícone do app (Widget próprio, não QLabel) ----
        ...
        # ---- Título (fallback se não houver ícone) ----
    ...
    # ---------- eventos sistêmicos: re-render em troca de DPI/tela ----------
    def event(self, e):
        if e.type() in (QEvent.ApplicationPaletteChange,
